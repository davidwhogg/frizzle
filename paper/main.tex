...
\newcommand{\modified}[1]{\textcolor{Emerald}{\textbf{#1}}}

\begin{document}

\title{\large \name: Combining spectra or images by forward modeling}

\author[0000-0003-0174-0564]{Andrew R. Casey}\altaffiliation{Authorship order was resolved by a game of \emph{Chicken} that lasted \emph{two years}: the first author would be whoever cracked and responded to the (perfectly reasonable and very helpful) referee report.}
...
We demonstrate the method with a small data example and provide open-source code for re-use.
\end{abstract}
...
We will assume that there are $N$ \modified{$> 1$} observations (epochs) of the same star (or maybe of different stars or quasars, say, that are assumed to be more-or-less identical, intrinsically, but let's think of a single star for now).
Each of these observations $i$ (with $1\leq i\leq \modified{N}$) produces a one-dimensional observed (noisy) image \modified{$\mathbf{y}_i$}.
The \modified{$\mathbf{y}_i$} can be thought of as $M_i$-vectors in what follows, \modified{with scalar components $y_{i,k}$ for $1\leq k\leq M_i$,} where $M_i$ is the number of pixels in spectrum $i$.
\modified{Each epoch can have a different length vector of pixel values.}
The different observations are made at different relative velocities (star minus spectrograph) such that the images are shifted by a different shift \modified{$\Delta\mathbf{x}_i$} relative to the detector or extracted wavelength grid.

\paragraph{Shift operators}
Because the shifts might be non-linear, the shifts \modified{$\Delta\mathbf{x}_i$} won't be pure numbers but something more like shift operators.
That is, if there were a true spectrum $\tilde{y}$, each individual noisy image \modified{$\mathbf{y}_i$} would be related to that true spectrum by a shift operator acting on that true spectrum, plus noise.
...
We will assume that, for every $M_i$-pixel image \modified{$\mathbf{y}_i$} we also have a $M_i$-vector (or list) \modified{$\mathbf{x}_i$} of pixel positions, such that each element \modified{$x_{i,k}$} of \modified{$\mathbf{x}_i$} gives an accurate and precise value for the wavelength (or position in the spectrograph, in some settings) for each pixel of the image \modified{$\mathbf{y}_i$}.
That is, the device is well calibrated, and we know all the housekeeping data we need for image \modified{$\mathbf{y}_i$}.
...
In what follows, each spectrum \modified{$\mathbf{y}_i$} is given an associated (but possibly inaccurate) noise-variance tensor $C_i$.
If \modified{$\mathbf{y}_i$} is an $M_i$-vector, then $C_i$ must be a nonnegative-definite $M_i\times M_i$ 2-tensor.
...
Occasionally (or frequently) some of the pixels of a spectrum \modified{$\mathbf{y}_i$} might be bad---affected by cosmic rays or electronics issues---such that there is a bad-pixel mask \modified{$\mathbf{b}_i$ with components $b_{i,k}$} associated with each spectrum \modified{$\mathbf{y}_i$}.
...
\paragraph{Data gaps}
There is no assumption here that all the \modified{$\mathbf{y}_i$} have the same number of pixels $M_i$, nor that the pixel grid is in any sense uniform or complete.
...
Instead we will assume that there is a relatively constant pixel-convolved line-spread function (PCLSF), constant both in time, and constant with respect to the shifts \modified{$\Delta\mathbf{x}_i$} we see in our data set.
...
It is not band-limited in that the photon noise in the device is independent (or close to independent) from pixel to pixel, such that the noise contribution to each observation \modified{$\mathbf{y}_i$} has support at all spatial frequencies.
...
That is, we are assuming that the changes from observation to observation are solely from either the shifts \modified{$\Delta\mathbf{x}_i$} or else from effects that we consider to be noise.
...
In particular, we will assume that the images or spectra have been bias-corrected, sky-subtracted, flat-fielded, and tellurics-corrected sufficiently well that the different observations \modified{$\mathbf{y}_i$} can be seen as observations of the same underlying or intrinsic signal.
Alternatively, the input data might be continuum-normalized\modified{, pseudo-continuum-normalized, or flux-calibrated}.
Again, the assumption is that the different calibrated or processed observations are of the same underlying signal. \modified{No strict accuracy level is required in these pre-processing steps for \name\ to be executed, but the consistency of the combined spectrum will naturally depend on the upstream processing steps.}
...
\modified{That is, if different input spectra have different normalizations, the model for the combined spectrum can be multiplied by a normalization model, unique to each input spectrum, prior to evaluation of the likelihood \citep[e.g.,][]{Casey:2026}.}
...
The problem set-up is that there are $N$ spectral measurements \modified{$\mathbf{y}_i$} (with $1\leq i\leq N$), each of which is a $M_i$-pixel list of intensities (fluxes maybe)\modified{, with scalar components $y_{i,k}$ for $1\leq k\leq M_i$}.
Different observations $i$ might have different numbers $M_i$ of pixels.
The $M_i$ pixels are associated with an $M_i$-length list of positions (wavelengths or log-wavelengths) \modified{$\mathbf{x}_i$ with components $x_{i,k}$}.
Each measurement or observation $i$ is shifted by a shift operator \modified{$\Delta\mathbf{x}_i$}, which can be thought of as a length-$M_i$ list of pixel offsets or shifts \modified{with components $\Delta x_{i,k}$}, such that the ``rest frame'' or ``unshifted'' positions corresponding to the length-$M_i$ list of data \modified{$\mathbf{y}_i$ are the differences $x_{i,k} - \Delta x_{i,k}$}.
...
\modified{\begin{align}
    y_{i,k} &= f(x_{i,k} - \Delta x_{i,k};\theta) + \varepsilon_{i,k} \\
    f(x;\theta) &= \sum_{j=1}^P a_j\,g_j(x) ~,
\end{align}}
where \modified{$\varepsilon_{i,k}$ is the additive (but not yet specified) noise contribution to pixel $k$ of epoch $i$}, and each of the $P$ functions $g_j(x)$ is a basis function of the representation.

The noise model is additive (as noted above) and we will assume that the noise contributing to observation \modified{$\mathbf{y}_i$} is zero-mean and Gaussian, with a known $M_i\times M_i$ variance tensor $C_i$ (or its inverse $C_i^{-1}$).
...
If you additionally have a bad-pixel mask \modified{$\mathbf{b}_i$} associated with each observation \modified{$\mathbf{y}_i$}, the bad-pixel mask can be used simply to zero out the corresponding bad rows and columns of the inverse covariance matrix $C_i^{-1}$.
Importantly, the bad data get zeros in the \emph{inverse} covariance matrix, not the covariance matrix!

Because we are often dealing with a spectrograph with a (fairly) well defined spectral resolution ($\delta\lambda/\lambda$ roughly constant), and because we care a lot about Doppler shifts (which are uniform in log wavelength), it makes sense to think of the pixel positions \modified{$\mathbf{x}_i$} and shifts \modified{$\Delta\mathbf{x}_i$} as being in log-wavelength (or ln-wavelength) units.
...
Now for the method:
We append all of the observations \modified{$\mathbf{y}_i$} into one huge $M$-pixel list $Y$, where $M=\sum_{i=1}^N M_i$.
We make a $M\times M$ block-diagonal matrix $C^{-1}$ from all the individual inverse variance matrices $C_i^{-1}$.
We make a $M\times P$ design matrix $X$ which is the evaluation of all the $P$ basis functions at all the \modified{rest frame} pixel locations \modified{$x_{i,k} - \Delta x_{i,k}$}.
...
\modified{
    Fitting a Fourier basis to non-uniformly sampled data by least-squares has a long history in least-squares spectral analysis \citep{vanicek1971, craymer1998} and the closely related Lomb--Scargle periodogram \citep{lomb1976, scargle1982}. What is specific to our setting is the multi-epoch, shift-operator framing and the goal of returning a combined spectrum on a pixel grid rather than a periodogram.
}
...
Putting these things together, the output spectrum $y_\star$ can be thought of as just a kind of linear weighted averaging operation on the input spectra \modified{$\mathbf{y}_i$} (which have been packed into $Y$) according to
\begin{align}
    y_\star &= A_\star\,Y \\
...
The information-theoretic answer is that the \modified{covariance matrix $C_\star$} delivering the uncertainties on $y_\star$ is
\modified{
\begin{align}
    C_\star &= X_\star\,(X^\top\,C^{-1}\,X)^{-1}\,X_\star^\top ~.\label{eq:outcovar}
\end{align}
...
And although the equations like \eqref{eq:xtxinvxty} show inverses, it is almost never a good idea to use the native linear-algebra inverse function \texttt{inv()} in any implementation of this method.
...
\modified{For example, the implementation associated with this work uses the \texttt{finufft} package (\citealt{finufft}) because we have a Fourier basis, and \texttt{JAX} \citep{jax} for automatic differentiation and just-in-time compilation, via the \texttt{jax-finufft} bindings \citep{jax-finufft}.}
...
The reason for this is \modified{the peak of the ``double descent'' curve at $P\approx M$, which de-stabilizes linear fitting} (\citealt{doubledescent}).
...
\modified{We arbitrarily assumed a mean density (Poisson rate) of 2$\times{}10^4$ spectral lines per unit in $x$ (natural logarithm of wavelength).}
...
Each spectral epoch was given a velocity (Doppler shift) by putting the epochs equally spaced on a sinusoidal variation with a semi-amplitude of $30\,\kmps$ (to emulate the motion of the spectrograph with respect to the Solar System barycenter). \modified{The toy data set only constructed once, without adjustment: no tuning or selection was performed in order to make the data more or less favorable to a given method.} \modified{Because the shifts in this demonstration are pure Doppler shifts, $\Delta x_{i,k}$ is constant in $k$ for each epoch $i$.}
...
\modified{We were judicious in the choice of which pixels to include, as \name\ is not robust to outlier pixels, which motivates a conservative approach to bad-pixel masking. This is no different to many other data analysis situations in astrophysics, in that outliers (measurements not drawn from your model) will have a deliterious effect on your results. One approach to this would be to shift spectra to rest-frame, interpolate, and perform sigma-clipping to identify outliers. An alternative would be to use a robust regression method, such as iteratively re-weighted least squares \citep{IRLS}, to downweight outliers in the forward modeling procedure. We leave these extensions to future work. Witout proper care, a 5-$\sigma$ outlier value in the input data would have a 1-$\sigma$ effect on the output spectrum if there are fewer than something like 25 epochs.}
...
The residuals look stationary and uncorrelated (more on this below).

\paragraph{Standard Practice(tm)}
In standard practice, the individual spectra \modified{$\mathbf{y}_i$} are interpolated to rest-frame spectra \modified{$\mathbf{y}'_i$} using an interpolator.
...
The interpolated spectra \modified{$\mathbf{y}'_i$} are, by construction, all on the same rest-frame wavelength grid $x_\star$ so they can be averaged, with a straight average, a median, an inverse-variance-weighted mean, or any more sophisticated algorithm.
Here we choose \modified{an inverse-weighted mean}, where we zero out the contributions of any bad pixels in that mean.


In Standard Practice(tm) \modified{and in \name}, this censorship of the bad pixels requires some attention:
Naive interpolation of an integer (binary actually) bad-pixel mask \modified{$\mathbf{b}_i$} will not deliver an integer mask \modified{$\mathbf{b}'_i$}.
The conservative move is to naively interpolate \modified{$\mathbf{b}_i$} (e.g., by a cubic spline) and then zero out any pixels that are significantly below unity.
...
\modified{The scale of the induced correlation matches the interpolation stencil: a cubic spline uses four adjacent input pixels, so noise in one input pixel leaks into roughly four output pixels in the well-sampled case, and roughly eight output pixels in the poorly-sampled case (where the input spacing of $2/R$ is twice the output spacing of $1/R$), in agreement with the shapes of the Standard Practice(tm) curves in Figure~\ref{fig:noise}.}
However, it is interesting to ask \emph{why} the output \name\ combined spectrum shows uncorrelated noise.
It won't in general: If the number of epochs is small, or the coverage in the wavelength domain is spotty, there will in general be pixel-to-pixel covariances in the uncertainties on the combined spectrum \modified{(e.g., particularly in the case of a single epoch spectrum that is under-sampled)}.
However, if the number of epochs is large and the distribution of shift operators \modified{$\Delta\mathbf{x}_i$} is good, as it is in these experiments, the off-diagonal elements of the output combined-spectrum covariance matrix given in \eqref{eq:outcovar} will tend towards zero.
...
\modified{In a similar way, per-epoch continuum-normalization errors, which are smooth functions of wavelength by construction, can only contribute power at spatial frequencies well below the pixel-to-pixel scale, and so cannot induce the kind of small-lag pixel covariances shown in Figure~\ref{fig:noise} for Standard Practice(tm); any such errors instead contribute to the variance at large lags and are common to both \name\ and Standard Practice(tm).}
\modified{If the sampling in the input data are perfectly sampled, where one epoch is sampled precisely half a pixel offset from the first, and the next is precisely half pixel from the first, then in these situations standard interpolation will not suffer from correlated noise, because interpolation methods are specifically designed for regular sampling patterns. However, such situations are rare: it would require a stable spectrograph to observe a star with a stable relative velocity that precisely compensated for the difference in barycentric motion between two (and only two) epochs.}
...
That is, once the (now more complicated) shift operators \modified{$\Delta\mathbf{x}_i$} are known, the problem proceeds by the same mathematics.
...
\modified{The forward-modeling step in \name{} is fast in practice. Figure~\ref{fig:speed} compares the wall-clock cost of \name{} against linear and cubic interpolation across a range of input sizes $M$; \name{} is overhead-dominated at small $M$ and scales comparably to cubic interpolation at large $M$, so computational cost need not be a barrier to adoption. Naively the cost of \name\ is expected to be larger than its Standard Practice(tm) counterparts, but is accelerated by the use of non-uniform fast Fourier transform algorithms and just-in-time compilation. The overhead-dominated costs at small $M$ are consistent with performance of non-uniform fast Fourier transforms. The code implementation that accompanies this work is written in \texttt{JAX}, allowing for execution on graphical processing units (GPUs). However, for the purpose of benchmarking, all timings were performed on a CPU.}

\begin{figure}[t!]
    \begin{mdframed}\begin{center}
    \includegraphics[width=\figurewidth]{frizzle_speed.pdf}\\
    \end{center}
    \caption{Computational cost (in CPU core seconds) for computing a combined spectrum and associated uncertainties with \name{}, compared to linear and cubic interpolation. Linear and cubic interpolation scale linearly with the total number of input pixels $M$. \name\ is overhead dominated at $M < 10^5$, and shows marginally sub-linear scaling at larger $M$, comparable to the cost of cubic interpolation. All timings were repeated, taking the best of repeated trials. \name\ was executed on a CPU, but can be readily executed on graphical processors.\label{fig:speed}}
    \end{mdframed}
\end{figure}
...
We have assumed that we know the shifts \modified{$\Delta\mathbf{x}_i$} exactly.
In practice, we rarely do, or maybe more accurately: We rarely know them as well as we could know them, in principle.
When the shifts \modified{$\Delta\mathbf{x}_i$} are not known, or not known accurately enough, we recommend a simultaneous-fitting procedure, in which the shifts are learned at the same time as the combined spectrum is estimated or inferred.
...
\modified{We also assumed that there are $N > 1$ epochs, but in practice this is not always true. When there is only one epoch, \name\ still has utility, albeit limited. If there are no bad pixels then the number of modes $M_\star$ is equal to the number of input pixels $M$. In this case, the forward model is just a change of basis, and the output spectrum is just a re-representation of the input spectrum. If there are bad pixels, then the number of modes $M_\star$ is less than the number of input pixels $M$, which can cause the output spectrum to have high-frequency ringing. Usually these ringing artifacts are merely cosmetic, in that the covariance matrix $C_\star$ has large variances at the pixels affected by ringing, but such scenarios dance on the edge of numerical stability.}

All code used for this project is available at \url{https://github.com/davidwhogg/frizzle} \modified{and is registered in the Python Package Index as \texttt{frizzle}}. \modified{This repository also includes code for generating the fake data sets, and for performing the experiments described above, as well as additional experiments thoughtfully requested during the review process.}
...
\modified{\texttt{jax-finufft} \citep{jax-finufft}}.
...
It is a pleasure to thank \modified{the anonymous referee for their detailed review, which helped us to improve this manuscript. We also thank}
    ...
    and the Inference Group at Monash University
...
\bibitem[Bradbury et al.(2018)]{jax} Bradbury, J., Frostig, R., Hawkins, P., Johnson, M.~J., Leary, C., Maclaurin, D., Necula, G., Paszke, A., VanderPlas, J., Wanderman-Milne, S., \& Zhang, Q.\ 2018, JAX: composable transformations of Python+NumPy programs, http://github.com/google/jax
\bibitem[Candes \& Romberg(2007)]{candes} Candes, E., \& Romberg, J., 2007, Inverse problems, 23, 969.
\bibitem[Casey et al.(2026)]{Casey:2026} Casey, A.~R., Wheeler, A., Bedell, M., Hogg, D.~W., Saydjari, A., \& Zhao, L.\ 2026, arXiv:2601.19973. doi:10.48550/arXiv.2601.19973
\bibitem[Craymer(1998)]{craymer1998} Craymer, M.~R.\ 1998, PhD thesis, University of Toronto.
\bibitem[Foreman-Mackey \& Garrison(2021)]{jax-finufft} Foreman-Mackey, D., \& Garrison, L.\ 2021, jax-finufft: JAX bindings to the Flatiron Institute Non-uniform Fast Fourier Transform (FINUFFT) library, \url{https://github.com/flatironinstitute/jax-finufft}
...
\bibitem[Hilder et al.(2025)]{Hilder:2025} Hilder, T., Casey, A.~R., Price, D.~J., Pinte, C., et al.\ 2025, \apjl, 984, L13. doi:10.3847/2041-8213/adc435
...
\bibitem[Holland \& Welsch(1977)]{IRLS} Holland, P.~W., \& Welsch, R.~E.\ 1977, Communications in Statistics - Theory and Methods, 6, 813. doi:10.1080/03610927708827533
...
\bibitem[Lomb(1976)]{lomb1976} Lomb, N.~R.\ 1976, \apss, 39, 447. doi:10.1007/BF00648343
...
\bibitem[Ravasi \& Vasconcelos(2020)]{pylops} Ravasi, M., \& Vasconcelos, I.\ 2020, SoftwareX, 11, 100361. doi:10.1016/j.softx.2019.100361
\bibitem[Scargle(1982)]{scargle1982} Scargle, J.~D.\ 1982, \apj, 263, 835. doi:10.1086/160554
\bibitem[Tsalmantza \& Hogg(2012)]{hmf} Tsalmantza, P. \& Hogg, D.~W.\ 2012, \apj, 753, 122. doi:10.1088/0004-637X/753/2/122
\bibitem[Vaní{\v{c}}ek(1971)]{vanicek1971} Vaní{\v{c}}ek, P.\ 1971, \apss, 12, 10. doi:10.1007/BF00656134
