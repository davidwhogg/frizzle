...
That is, if different input spectra have different normalizations, the model for the combined spectrum can be multiplied by a normalization model, unique to each input spectrum, prior to evaluation of the likelihood \citep[e.g.,][]{Casey:2026}.
...
All code used for this project is available at \url{https://github.com/davidwhogg/NoDataInterpolation} \modified{and is registered in the Python Package Index as \texttt{frizzle}}. \modified{This repository also includes code for generating the fake data sets, and for performing the experiments described above, as well as additional experiments thoughtfully requested during the review process.}
